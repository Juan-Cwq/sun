    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        if (data.status === 'SUCCESS' && data.audioUrl) {
          ...
          // Save to localStorage with cover art
          const song: Song = {
            id: taskId,
            title: tracks[0].title || 'Your Song',
        ...
        <div className="max-w-2xl w-full bg-white rounded-2xl shadow-2xl p-8 relative z-10 border border-gray-100">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex-1 flex justify-end">
              <span className="text-xs text-gray-500 italic">Created by Cadenza</span>
            </div>
          </div>
              ...
              <div className="mt-3 p-4 bg-gradient-to-br from-blue-50 to-teal-50 rounded-lg text-left space-y-3 border border-[#4ECDC4]/30">
                <div>
                  ...
                  <ul className="text-xs text-gray-700 space-y-1 ml-4">
                    <li>• <strong>Use YouTube references</strong> to show the style/vocals you want</li>
          ...
          <div className="space-y-4">
            {/* YouTube Reference Field */}
            <div className="space-y-2">
              <label className="text-sm font-semibold text-gray-700 flex items-center gap-2">
                <span>🎬</span>
                <span>YouTube Reference (Optional)</span>
              </label>
              <input
                type="text"
                value={youtubeReference}
                onChange={(e) => setYoutubeReference(e.target.value)}
                placeholder="Paste a YouTube link for style inspiration..."
                ...
                disabled={loading}
              />
              {youtubeReference && extractYouTubeId(youtubeReference) && (
                <div className="bg-gradient-to-r from-purple-50 to-blue-50 border border-purple-200 rounded-lg p-3">
                  ...
                  <div className="aspect-video rounded overflow-hidden shadow-md">
                    <iframe
                      width="100%"
                      height="100%"
                      src={`https://www.youtube.com/embed/${extractYouTubeId(youtubeReference)}`}
              ...
              {youtubeReference && !extractYouTubeId(youtubeReference) && (
                <p className="text-xs text-orange-600">⚠️ Invalid YouTube URL. Try a link like: https://youtube.com/watch?v=...</p>
              )}
            </div>
          ...
          <div className="mt-6 pt-4 border-t border-gray-200 text-center">
            <p className="text-xs text-gray-500">
              Created with ❤️ by <span className="font-semibold text-[#556FB5]">Cadenza</span>
            </p>
          </div>
        </div>
      </main>
